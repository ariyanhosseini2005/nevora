...
const manifestPath = path.join(outputDirectory, "manifest-v005.json");

const startFrame = 204;
const previousEndFrame = 491;
const endFrame = 539;
...
function beatForFrame(frame) {
  ...
  if (frame <= 491) return "roast-development";
  return "grinder-transition";
}
...
async function assertCleanDestination() {
  ...
  if (existing.length > 0) {
    throw new Error(
      `Web-frame v005 destination is not empty: ${outputDirectory}. Refusing to overwrite.`,
    );
  }
}
...
for (let frame = startFrame; frame <= endFrame; frame += 1) {
  ...
  frames.push({
    ...
    web: `/images/journey/frames-v005/${webName(frame)}`,
    bytes: (await stat(output)).size,
    build_mode: build.mode,
  });
  process.stdout.write(
    `${build.mode === "rendered" ? "Built" : "Reused"} web F${frame} (${frames.length}/336)\n`,
  );
}
