...
const reviewPath = path.join(storyboardRoot, "drying-review-sequence-v2.jpg");
const metricsPath = path.join(
  storyboardRoot,
  "drying-continuity-metrics-v2.json",
);

const startFrame = 300;
const endFrame = 347;
const proxySize = 512;
const correctedFrames = new Set([
...
function versionedProxyName(frame, version) {
...
function activeVersion(frame) {
  return correctedFrames.has(frame) ? 2 : 1;
}

function activeProxyPath(frame) {
  return path.join(
    proxyDirectory,
    versionedProxyName(frame, activeVersion(frame)),
  );
}
...
async function createCorrectedProxies() {
  await mkdir(rejectedDirectory, { recursive: true });
  for (const frame of correctedFrames) {
    const source = path.join(
      correctionDirectory,
      `nevora-one-take_f${padded(frame)}_v002.png`,
    );
    const output = path.join(proxyDirectory, versionedProxyName(frame, 2));
    if (await exists(output)) {
      throw new Error(`Refusing to overwrite existing corrected proxy: ${output}`);
    }
    ...
    if (!(await exists(superseded))) {
      throw new Error(`Missing superseded proxy: ${superseded}`);
    }
    if (await exists(archived)) {
      throw new Error(`Refusing to overwrite archived proxy: ${archived}`);
    }
    await rename(superseded, archived);
    process.stdout.write(`Activated F${frame} v002; archived v001.\n`);
  }
}
...
async function buildMetrics() {
  const sequence = [
    ...
    })),
    {
      ...
      input: path.join(
        ...
        "nevora-one-take_f0348_anchor-v002.png",
      ),
  ...
  if (activeFiles.length !== 48) {
    throw new Error(
      `Expected exactly 48 active proxies, found ${activeFiles.length}.`,
    );
  }
  ...
  const report = {
    ...
    corrected_frames: [...correctedFrames],
    following_anchor: {
      frame: 348,
      version: 2,
