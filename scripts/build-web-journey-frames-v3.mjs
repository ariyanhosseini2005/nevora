...
const manifestPath = path.join(outputDirectory, "manifest-v003.json");

const startFrame = 204;
const previousEndFrame = 395;
const endFrame = 443;
...
async function assertCleanDestination() {
  ...
  if (existing.length > 0) {
    throw new Error(
      `Web-frame v003 destination is not empty: ${outputDirectory}. Refusing to overwrite.`,
    );
  }
}
...
for (let frame = startFrame; frame <= endFrame; frame += 1) {
  ...
  frames.push({
    ...
    web: `/images/journey/frames-v003/${webName(frame)}`,
    bytes: (await stat(output)).size,
    build_mode: build.mode,
  });
  process.stdout.write(
    `${build.mode === "rendered" ? "Built" : "Reused"} web F${frame} (${frames.length}/240)\n`,
  );
}
